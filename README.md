...
一个基于真实 Markdown 简历资料的招聘问答 Demo。回答链路为：知识库切片 → OpenAI Embedding 检索 → ChromaDB → LLM 回答。
...
1. 复制 `backend/.env.example` 为 `backend/.env`，填入 `OPENAI_API_KEY`。
2. 在项目根目录执行：
...
2. 添加加密环境变量 `OPENAI_API_KEY`，仅用于 Production/Preview。
3. Deploy。Vercel 会将 `api/index.py` 识别为 FastAPI 函数；其 `index.py` 入口自动处理 `/api/*` 路径，因此不应额外把 `/api/chat` rewrite 到其他路径。
